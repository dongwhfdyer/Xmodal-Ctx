    ...
    for batchInd, batch in enumerate(tqdm(data_loader)):
        ...
        with torch.no_grad():
            if timer:
                timer.tic()
            if bbox_aug:  # False
                # batch_size boxes. Each boxlist has 0-100 boxes. Each box is a list of 4 numbers: [x1, y1, x2, y2]
...
def convert_predictions_to_tsv(predictions, dataset, output_folder,
    ...
    def gen_rows():
        for idx, prediction in sorted(predictions.items()):
            ...
            bfea = prediction.get_field("box_features").numpy()
            #---------kkuhn-block------------------------------
...
def inference(
    ...
    if output_folder and os.path.isfile(os.path.join(output_folder, output_pth_name)):  # False
    ...
    # predictions = _accumulate_predictions_from_multiple_gpus(predictions, cfg.TEST.GATHER_ON_CPU)  # kuhn edited
    ...
    if output_folder and cfg.TEST.SAVE_RESULTS_TO_TSV:  # True
        ...
        convert_predictions_to_tsv(  # kuhn: delete
