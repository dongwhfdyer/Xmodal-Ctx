...
class CaptioningModel(Module):
    ...
    def __test(self, visual: utils.TensorOrSequence, max_len: int, eos_idx: int, **kwargs) -> utils.Tuple[torch.Tensor, torch.Tensor]:
    ...
    def __sample_rl(self, visual: utils.TensorOrSequence, max_len: int, **kwargs) -> utils.Tuple[torch.Tensor, torch.Tensor]:
        b_s = utils.get_batch_size(visual)
        outputs = []
        log_probs = []
    ...
    def beam_search(self, img, obj, ctx, max_len: int, eos_idx: int, beam_size: int, out_size=1,
